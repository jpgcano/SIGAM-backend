...
class IaJobsService {
    constructor({ repuestoModel, alertaModel, assetModel, ticketModel, maintenanceModel, openAiProvider, iaConfig } = {}) {
    ...
    async reprocessTicketsExternal({
        ...
        if (!this.iaConfig.enabled) {
            return { ok: false, reason: 'IA deshabilitada', processed: 0, updated: 0 };
        }

        if (!this.openAiProvider?.isAvailable?.()) {
            return { ok: false, reason: 'Proveedor externo no disponible', processed: 0, updated: 0 };
        }
